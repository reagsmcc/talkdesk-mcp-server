...
const ORDERS = {
  "ORD-78901": { id: "ORD-78901", customerId: "C-1001", status: "in_transit", createdAt: "2025-03-10T09:22:00Z", estimatedDelivery: "2025-03-20", total: 249.99, items: [{ sku: "SKU-A1", name: "Wireless Headphones Pro", qty: 1, price: 199.99 }, { sku: "SKU-B3", name: "USB-C Charging Cable", qty: 2, price: 24.99 }], carrier: "FedEx", trackingNumber: "FX-9876543210", shippingAddress: "123 Maple St, Austin TX 78701" },
...
const TOOLS = [
  { name: "lookup_order", description: "Get full order details by order ID", inputSchema: { type: "object", properties: { order_id: { type: "string", description: "Order ID e.g. ORD-78901" } }, required: ["order_id"] } },
  { name: "lookup_customer", description: "Find a customer by email, phone, or customer ID", inputSchema: { type: "object", properties: { identifier: { type: "string", description: "Email, phone, or customer ID" } }, required: ["identifier"] } },
  { name: "list_orders", description: "List recent orders for a customer", inputSchema: { type: "object", properties: { customer_id: { type: "string" }, limit: { type: "integer", default: 5 } }, required: ["customer_id"] } },
  { name: "update_order_status", description: "Update an order status", inputSchema: { type: "object", properties: { order_id: { type: "string" }, new_status: { type: "string", enum: ["processing","in_transit","delivered","cancelled","on_hold"] }, reason: { type: "string" } }, required: ["order_id","new_status"] } },
  ...
  { name: "check_product_stock", description: "Check inventory for a product SKU", inputSchema: { type: "object", properties: { sku: { type: "string" } }, required: ["sku"] } },
  { name: "create_ticket", description: "Create a support ticket", inputSchema: { type: "object", properties: { customer_id: { type: "string" }, subject: { type: "string" }, description: { type: "string" }, priority: { type: "string", enum: ["low","normal","high","urgent"], default: "normal" }, category: { type: "string" } }, required: ["customer_id","subject","description"] } },
];
...
const server = http.createServer((req, res) => {
  ...
  if (url.pathname === "/health") {
    ...
    res.end(JSON.stringify({ status: "ok", server: "talkdesk-order-mcp", version: "1.0.0", tools: TOOLS.length }));
    return;
  }

  // Handle all /sse requests
  if (url.pathname === "/sse") {
    // POST to /sse - handle inline (Talkdesk connection test)
    if (req.method === "POST") {
      let body = "";
      req.on("data", c => body += c);
      req.on("end", () => {
        let msg;
        try { msg = body ? JSON.parse(body) : { jsonrpc: "2.0", method: "ping", id: 1 }; }
        ...
        res.writeHead(200, { "Content-Type": "application/json" });
        res.end(JSON.stringify(response || { jsonrpc: "2.0", id: msg.id, result: {} }));
      });
      return;
    }

    // GET to /sse - open SSE stream
    if (req.method === "GET") {
